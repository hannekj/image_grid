...
class LayoutOutlinePainter extends CustomPainter {
  ...
  void _paintFilmStrip(
    ...
    canvas.drawRect(Offset.zero & size, Paint()..color = AppTheme.cream);
    final cellPaint = Paint()..color = cellColor;
    final ratio = FilmStripMetrics.lengthFor(frames);

    final short = horizontal
        ? math.min(size.height * 0.52, size.width * 0.94 / ratio)
        : math.min(size.width * 0.52, size.height * 0.94 / ratio);

    canvas.save();
    canvas.translate(size.width / 2, size.height / 2);
    canvas.rotate(horizontal ? -0.022 : 0.026);

    final local = Rect.fromCenter(
      center: Offset.zero,
      width: horizontal ? short * ratio : short,
      height: horizontal ? short : short * ratio,
    );
    ...
    final radius = Radius.circular(short * FilmStripMetrics.frameRadius);
    final windowShort = short - across * 2;
    final windowLong =
        ((horizontal ? local.width : local.height) -
                along * 2 -
                gutter * (frames - 1)) /
            frames;

    for (var i = 0; i < frames; i++) {
      final offset = along + i * (windowLong + gutter);
      final window = horizontal
          ? Rect.fromLTWH(
              local.left + offset,
              local.top + across,
              windowLong,
      ...
      canvas.drawRRect(RRect.fromRectAndRadius(window, radius), cellPaint);
    }
